...
import com.zyl.melife.camera.view.AnimationTextView;
import com.zyl.melife.camera.view.AutoFitSurfaceView;
import com.zyl.melife.camera.view.CameraGridView;
import com.zyl.melife.camera.view.FocusView;
import com.zyl.melife.camera.view.GestureView;
import com.zyl.melife.camera.view.IsoView;
import com.zyl.melife.camera.view.MenuLayout;
import com.zyl.melife.camera.view.callback.OnLayoutItemClickListener;
...
public class CameraActivity extends AppCompatActivity implements SurfaceHolder.Callback,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSharePreferencesUtils = SharePreferencesUtils.getInstance(getApplicationContext());
    ...
    private void initData() {
        ...
        mIsSaving = mSharePreferencesUtils.getCameraSaveSetting();
        if (mIsSaving) {
            mCameraId = mSharePreferencesUtils.getCameraSaveCameraId();
            mFlashState = mSharePreferencesUtils.getCameraSaveFlash();
            mTimerState = mSharePreferencesUtils.getCameraSaveTimer();
            mSoundOpen = mSharePreferencesUtils.getCameraSoundOpen();
            mSizeState = mSharePreferencesUtils.getCameraPreviewRatioDefault();
            mExposureCompensation = mSharePreferencesUtils.getCameraExposureCompensation();
            mLocationOpen = mSharePreferencesUtils.getCameraLocation();
            mWBState = mSharePreferencesUtils.getCameraWhiteBalance();
            mGridOpen = mSharePreferencesUtils.getCameraGridOpen();
        }
        getCameraRotation();
    }
    ...
    private void getCameraRotation() {
        switch (mCameraId) {
            case Const.CAMERA_BACK:
                mCameraRotation = mSharePreferencesUtils.getCameraBackRotation();
                break;
            case Const.CAMERA_FRONT:
                mCameraRotation = mSharePreferencesUtils.getCameraFrontRotation();
                break;
        }
    }
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        ...
        Size size = null;
        try {
            size = mSharePreferencesUtils.getPictureSize(mCameraId);
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    protected void onPause() {
        ...
        if (mIsSaving) {
            mSharePreferencesUtils.setCameraSaveTimer(mTimerState);
            mSharePreferencesUtils.setCameraSaveCameraId(mCameraId);
            mSharePreferencesUtils.setCameraSaveFlash(mFlashState);
            mSharePreferencesUtils.setCameraSoundOpen(mSoundOpen);
            mSharePreferencesUtils.setCameraPreviewRatio(mSizeState);
            mSharePreferencesUtils.setCameraExposureCompensation(mExposureCompensation);
            mSharePreferencesUtils.setCameraLocation(mLocationOpen);
            mSharePreferencesUtils.setCameraWhiteBalance(mWBState);
            mSharePreferencesUtils.setCameraGridOpen(mGridOpen);
        }
    }
    ...
    public void onBeginFocusing(float x, float y) {
        ...
        float leftMargin;
        if (x <= ViewUtil.getScreenWidth(this) / 2) {
        ...
        switch (item) {
            ...
            case Const.LAYOUT_MAIN_CAPTURE:
                ...
                if (mTimerNumber != 0 && !mWindowTextView.isCountDown()) {
                    ...
                    mCameraModel.capture(mSoundOpen, mSizeState, mSharePreferencesUtils.getCameraMirrorOpen());
                }
                break;
            case Const.LAYOUT_PERSONAL_TIMER_0:
                mTimerState = item;
    ...
    private void initCameraNumberAndPictureSize() throws JSONException {
        List<Size> initSizeList = mSharePreferencesUtils.getPictureSizes(Const.CAMERA_BACK);
        if (initSizeList != null) {
            return;
        }
        //暂时用Camera的方法
        int total = Camera.getNumberOfCameras();
        mSharePreferencesUtils.setCameraNumber(total);
        int[] cameraIds;
        if (total == 0) {
            cameraIds = new int[0];
        } else if (total == 1) {
        ...
        for (int i = 0; i < cameraIds.length; i++) {
            try {
                ...
                mSharePreferencesUtils.setPictureSizes(String.valueOf(cameraIds[i]), sizeList);
                Size suitableSize = sizeList.get(0);
                mSharePreferencesUtils.setPictureSize(String.valueOf(cameraIds[i]), suitableSize);
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
